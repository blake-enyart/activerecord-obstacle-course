...
# ----------------------
  ...
  it '9. finds orders for a user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_user_3 = Order.includes(:user).where(users: {id: @user_3.id})
    # ------------------------------------------------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Item.includes(:orders).where(orders: {id: @order_3.id})
    # ------------------------------------------------------------
...
# ========================
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.includes(:items).where(items: {id: @item_8.id}).distinct(:name).pluck(:name).sort
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.includes(:user).where(users: {id: @user_3.id})[2].items.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.includes(:user).where(users: {id: @user_3.id}).average(:amount)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = Order.includes(:user).where.not(users: {id: @user_2.id}).sum(:amount)
    # -----------------------------------------------------------
  ...
  it '23. returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(items: :order_items)
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(items: :order_items)
                  .where(user_id: @user_2.id, 'items.id': @item_4.id)
                  .distinct(:id)
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.joins(:orders).distinct(:id).order(:id)
    # ---------------------------------------------------------------
